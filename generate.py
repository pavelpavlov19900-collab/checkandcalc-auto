...
try:
   ...
   prompt_text = f"Write a massive, highly detailed SEO article in English about: {topic_title}. CRITICAL RULES: 1. Use a unique writing style adapted to the topic. NEVER start two articles with the same phrasing. 2. START with the main title in <h1> tags. 3. IMMEDIATELY AFTER the <h1>, generate a <div class='ai-answer-box'><h2>Quick Answer (TL;DR)</h2><ul>...</ul></div> with 3-5 ultra-concise, highly factual bullet points summarizing the core answer using <strong> tags for key tools/entities. 4. Follow with an engaging Introduction, EXACTLY 5 to 7 main sections using <h2> tags, and a Conclusion. 5. You MUST write AT LEAST 400 WORDS under EACH <h2> section. 6. FORMAT STRICTLY IN HTML with <p>, <ul>, <li>, <strong> tags. 7. ABSOLUTELY NO MARKDOWN. 8. Include a dedicated <h2> section about tools or solutions. 9. MANDATORY: You must finish the article with a formal Conclusion and ensure all HTML tags are perfectly closed. Do not stop mid-sentence. Return ONLY the raw HTML body content starting with <h1>."
